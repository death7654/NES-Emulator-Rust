...
impl PPU {
    ...
    pub fn process_fetches_and_scrolling(&mut self, cartridge: &mut Cartridge) {
    ...
    pub fn ppu_read(&self, cartridge: &mut Cartridge, mut addr: u16) -> u8 {
        // PPU address bus is 14 bits wide
        addr &= 0x3FFF;

        match addr {
            // pattern tables
            0x0000..=0x1FFF => cartridge.mapper.as_mut().unwrap().ppu_read(addr),
    ...
    pub fn ppu_write(&mut self, cartridge: &mut Cartridge, mut addr: u16, data: u8) {
        ...
        match addr {
            ...
            0x0000..=0x1FFF => {
                cartridge.mapper.as_mut().unwrap().ppu_write(addr, data);
            }
    ...
    pub fn cpu_read(&mut self, cartridge: &mut Cartridge, addr: u16) -> u8 {
